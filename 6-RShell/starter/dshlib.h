...
#define EXE_MAX 64
#define ARG_MAX_CUSTOM 256  // Renamed to avoid conflict
#define CMD_MAX 8
#define CMD_ARGV_MAX (CMD_MAX + 1)
// Longest command that can be read from the shell
#define SH_CMD_MAX EXE_MAX + ARG_MAX_CUSTOM

typedef struct command
{
    char exe[EXE_MAX];
    char args[ARG_MAX_CUSTOM];
} command_t;
...
Built_In_Cmds exec_built_in_cmd(cmd_buff_t *cmd, int *last_status);  // Updated to match definition
